...
class ExcepcionStringNumero(ErrorFormato):
    def __init__(self,valor):
        self.mensaje_error = f"la entrada {valor}  solo consta de digitos, por favor modifiquela, muchas gracias."
        super().__init__(self.mensaje_error)

class ExcepcionEnteroString(ErrorFormato):
    def __init__(self,valor):
        self.mensaje_error=f"\n{valor} es un texto, por favor modifiquelo a un numero entero,muchas gracias." 
        super().__init__(self.mensaje_error)

class ExcepcionEnteroFloat(ErrorFormato):

    def __init__(self,valor):
        self.mensaje_error=f"\n{valor} es un numero flotante, por favor modifiquelo a un entero,muchas gracias ."
        super().__init__(self.mensaje_error)
